...
export async function createUser(id: string, email: string, walletAddress: string) {

  try {
    return await db.insert(user).values({ id, email, walletAddress, createdAt: new Date(), updatedAt: new Date() });
  } catch (error) {
